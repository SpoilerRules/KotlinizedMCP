...
public class SoundHandler implements IResourceManagerReloadListener, ITickable
{
    ...
    public void onResourceManagerReload(IResourceManager resourceManager) {
        ...
        for (String domain : resourceManager.getResourceDomains()) {
            try {
                for (IResource resource : resourceManager.getAllResources(new ResourceLocation(domain, "sounds.json"))) {
                    try (InputStream inputStream = resource.getInputStream()) {
                        Map<String, SoundList> soundMap = getSoundMap(inputStream);

                        for (Map.Entry<String, SoundList> entry : soundMap.entrySet()) {
                            String soundName = entry.getKey();
                            SoundList soundList = entry.getValue();
                            ResourceLocation soundLocation = new ResourceLocation(domain, soundName);

                            loadSoundResource(soundLocation, soundList);
                        }
    ...
    protected Map<String, SoundList> getSoundMap(InputStream stream) {
        Map<String, SoundList> soundMap = null;
    ...
    private void loadSoundResource(ResourceLocation location, SoundList sounds)
    {
        ...
        for (final SoundList.SoundEntry soundlist$soundentry : sounds.getSoundList())
        {
            ...
            switch (soundlist$soundentry.getSoundEntryType()) {
                ...
                case SOUND_EVENT -> isoundeventaccessor = new ISoundEventAccessor<>() {
                ...
                default -> throw new IllegalStateException("IN YOU FACE");
            }
