...
public class ServerListEntryNormal implements GuiListExtended.IGuiListEntry {
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected) {
        ...
        if (this.mc.gameSettings.touchscreen || isSelected) {
            ...
            if (this.owner.canEditServer(slotIndex)) {
                if (k1 < 16 && l1 < 16) {
            ...
            if (this.owner.canDeleteServer(slotIndex)) {
                if (k1 < 16 && l1 > 16) {
    ...
    private boolean isServerListVisible() {
        return true;
    }
     ...
     */
    public boolean mousePressed(int serverSlotIndex, int mouseX, int mouseY, int mouseButton, int mouseClickX, int mouseClickY) {
        if (mouseClickY <= 32) {
            if (mouseClickY > 16 && mouseClickY < 32 && this.isServerListVisible()) {
                this.owner.selectServer(serverSlotIndex);
                this.owner.connectToSelected();
                return true;
            }

            if (mouseClickY < 16 && mouseClickX < 16 && this.owner.canEditServer(serverSlotIndex)) {
                this.owner.editServer(serverSlotIndex, GuiScreen.isShiftKeyDown());
                return true;
            }

            if (mouseClickY < 16 && mouseClickX > 16 && this.owner.canDeleteServer(serverSlotIndex)) {
                this.owner.deleteServer(serverSlotIndex, GuiScreen.isShiftKeyDown());
                return true;
            }
        }
