...
public class GuiStreamUnavailable extends GuiScreen
{
    ...
    public void initGui()
    {
        ...
        {
            this.field_152323_r.addAll(this.fontRendererObject.listFormattedStringToWidth(this.field_152326_h.func_152561_a().getFormattedText(), (int)((float)this.width * 0.75F)));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        int i = Math.max((int)((double)this.height * 0.85D / 2.0D - (double)((float)(this.field_152323_r.size() * this.fontRendererObject.FONT_HEIGHT) / 2.0F)), 50);
        this.drawCenteredString(this.fontRendererObject, this.field_152324_f.getFormattedText(), this.width / 2, i - this.fontRendererObject.FONT_HEIGHT * 2, 16777215);

        for (String s : this.field_152323_r)
        {
            this.drawCenteredString(this.fontRendererObject, s, this.width / 2, i, 10526880);
    ...
    public static void checkStreamAvailabilityAndDisplayScreen(GuiScreen currentScreen)
    {
        ...
        if (!OpenGlHelper.framebufferSupported)
        {
            ...
            minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.NO_FBO, list));
        }
        else if (istream instanceof NullStream)
        {
            ...
            {
                minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.LIBRARY_ARCH_MISMATCH));
            }
            else
            {
                minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.LIBRARY_FAILURE));
            }
        }
        else if (!istream.func_152928_D() && istream.func_152912_E() == ErrorCode.TTV_EC_OS_TOO_OLD)
        {
            ...
            {
                case WINDOWS:
                    minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.UNSUPPORTED_OS_WINDOWS));
                    break;

                case OSX:
                    minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.UNSUPPORTED_OS_MAC));
                    break;

                default:
                    minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.UNSUPPORTED_OS_OTHER));
            }
        }
        else if (!minecraft.getTwitchDetails().containsKey("twitch_access_token"))
        {
            ...
            {
                minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.ACCOUNT_NOT_MIGRATED));
            }
            else
            {
                minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.ACCOUNT_NOT_BOUND));
            }
        }
        else if (!istream.func_152913_F())
        {
            ...
            {
                case INVALID_TOKEN:
                    minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.FAILED_TWITCH_AUTH));
                    break;

                case ERROR:
                default:
                    minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.FAILED_TWITCH_AUTH_ERROR));
            }
        }
        else if (istream.func_152912_E() != null)
        {
            ...
            minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.INITIALIZATION_FAILURE, list1));
        }
        else
        {
            minecraft.displayGuiScreen(new GuiStreamUnavailable(currentScreen, GuiStreamUnavailable.Reason.UNKNOWN));
        }
    }
