...
public class PacketBuffer extends ByteBuf {
    ...
    public String readStringFromBuffer(int maxLength) {
        int encodedLength = this.readVarIntFromBuffer();

        if (encodedLength > maxLength * 4) {
            throw new DecoderException("The received encoded string buffer length is longer than maximum allowed (" + encodedLength + " > " + maxLength * 4 + ")");
        } else if (encodedLength < 0) {
            throw new DecoderException("The received encoded string buffer length is less than zero! Weird string!");
        } else {
            ByteBuf byteBuf = this.readBytes(encodedLength); // Read the ByteBuf
            try {
                ...
                if (result.length() > maxLength) {
                    throw new DecoderException("The received string length is longer than maximum allowed (" + encodedLength + " > " + maxLength + ")");
                } else {
                    return result;
                }
            } finally {
                byteBuf.release(); // Release the ByteBuf after using it
            }
        }
    }
    ...
    public @NotNull String toString(int p_toString_1_, int p_toString_2_, Charset p_toString_3_) {
