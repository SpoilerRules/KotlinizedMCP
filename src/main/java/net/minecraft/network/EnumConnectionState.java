...
public enum EnumConnectionState {
    ...
    private static final Map<Class<? extends IPacket<?>>, EnumConnectionState> STATES_BY_CLASS = Maps.newHashMap();
    private final int id;
    private final Map<EnumPacketDirection, BiMap<Integer, Class<? extends IPacket>>> directionMaps;
    ...
    protected void registerPacket(EnumPacketDirection direction, Class<? extends IPacket> packetClass) {
    ...
    public Integer getPacketId(EnumPacketDirection direction, IPacket packetIn) {
        return (Integer) ((BiMap) this.directionMaps.get(direction)).inverse().get(packetIn.getClass());
    }

    public IPacket getPacket(EnumPacketDirection direction, int packetId) throws InstantiationException, IllegalAccessException, NoSuchMethodException, InvocationTargetException {
        Class oclass = (Class) ((BiMap) this.directionMaps.get(direction)).get(packetId);
        return oclass == null ? null : (IPacket) oclass.getDeclaredConstructor().newInstance();
    }
    ...
    public static EnumConnectionState getFromPacket(IPacket<?> packetIn) {
    ...
    static {
        for (EnumConnectionState enumconnectionstate : values()) {
            ...
            for (EnumPacketDirection enumpacketdirection : enumconnectionstate.directionMaps.keySet()) {
                for (Class<? extends IPacket> oclass : (enumconnectionstate.directionMaps.get(enumpacketdirection)).values()) {
                    ...
                    STATES_BY_CLASS.put((Class<? extends IPacket<?>>) oclass, enumconnectionstate);
                }
            }
        }
    }
