...
public class JsonToNBT {
    ...
    public static NBTTagCompound getTagFromJson(String jsonString) throws NBTException {
        ...
        } else if (countTopTags(jsonString) != 1) {
            throw new NBTException("Encountered multiple top tags, only one expected");
        } else {
            return (NBTTagCompound) parseJsonTag("tag", jsonString).parse();
        }
    }

    static int countTopTags(String p_150310_0_) throws NBTException {
        int i = 0;
        boolean flag = false;
        ...
        for (int j = 0; j < p_150310_0_.length(); ++j) {
            ...
            if (c0 == 34) {
                ...
                if (c0 == 125 || c0 == 93) {
                    int matchingBracket = c0 == 125 ? 123 : 91;
                    if (stack.isEmpty() || stack.pop() != matchingBracket) {
                        throw new NBTException("Unbalanced " + (c0 == 125 ? "curly" : "square") + " brackets " + (c0 == 125 ? "{}" : "[]") + ": " + p_150310_0_);
                    }
                } else {
                    if (stack.isEmpty()) {
                        ++i;
    ...
    static JsonToNBT.Any func_179272_a(String... p_179272_0_) throws NBTException {
        return parseJsonTag(p_179272_0_[0], p_179272_0_[1]);
    }

    static JsonToNBT.Any parseJsonTag(String name, String json) throws NBTException {
        json = json.trim();
        String value = json.substring(1, json.length() - 1);

        if (json.startsWith("{")) {
            JsonToNBT.Compound compound = new JsonToNBT.Compound(name);
            processJson(value, compound, true);
            return compound;
        } else if (json.startsWith("[") && !ARRAY_PATTERN.matcher(json).matches()) {
            JsonToNBT.List list = new JsonToNBT.List(name);
            processJson(value, list, false);
            return list;
        } else {
    ...
    static void processJson(String json, JsonToNBT.Any tag, boolean flag) throws NBTException {
        while (!json.isEmpty()) {
            String s = func_150314_a(json, flag);

            if (!s.isEmpty()) {
                ((JsonToNBT.List) tag).field_150492_b.add(parseJsonTag(s, flag));
            }

            if (json.length() < s.length() + 1) {
                break;
            }

            char c = json.charAt(s.length());

            if (c != 44 && c != 123 && c != 125 && c != 91 && c != 93) {
                throw new NBTException("Unexpected token '" + c + "' at: " + json.substring(s.length()));
            }

            json = json.substring(s.length() + 1);
        }
    }

    private static JsonToNBT.Any parseJsonTag(String json, boolean isObject) throws NBTException {
        String tagName = func_150313_b(json, isObject);
        String tagValue = func_150311_c(json, isObject);
        return func_179272_a(tagName, tagValue);
    }
    ...
    private static String func_179269_a(String p_179269_0_, int p_179269_1_) throws NBTException {
        Stack<Character> stackz = new Stack<>();
        int i = p_179269_1_ + 1;
        boolean flag = false;
        boolean flag1 = false;
        boolean flag2 = false;
    ...
    static class Primitive extends JsonToNBT.Any {
        ...
        public NBTBase parse() throws NBTException {
            try {
                Map<Pattern, Function<String, NBTBase>> patternMap = getPatternFunctionMap();

                String value = this.jsonValue.substring(0, this.jsonValue.length() - 1);

                for (Map.Entry<Pattern, Function<String, NBTBase>> entry : patternMap.entrySet()) {
                    if (entry.getKey().matcher(this.jsonValue).matches()) {
        ...
        @NotNull
        private static Map<Pattern, Function<String, NBTBase>> getPatternFunctionMap() {
            Map<Pattern, Function<String, NBTBase>> patternMap = new HashMap<>();
            patternMap.put(DOUBLE, value -> new NBTTagDouble(Double.parseDouble(value)));
            patternMap.put(FLOAT, value -> new NBTTagFloat(Float.parseFloat(value)));
            patternMap.put(BYTE, value -> new NBTTagByte(Byte.parseByte(value)));
            patternMap.put(LONG, value -> new NBTTagLong(Long.parseLong(value)));
            patternMap.put(SHORT, value -> new NBTTagShort(Short.parseShort(value)));
            patternMap.put(INTEGER, value -> new NBTTagInt(Integer.parseInt(value)));
            patternMap.put(DOUBLE_UNTYPED, value -> new NBTTagDouble(Double.parseDouble(value)));
            return patternMap;
        }
    }
}
